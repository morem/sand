...
public class MTop_API extends Object
{
    ...
    public Boolean UserNotifyPermit(String id)
    {
        ...
        String topsession = new MUserData().GetUserTopSession(id);
        if (topsession  == null){
            logger.error("Can't find the user's refresh_token");
            return false;
        }
        ...
        try {
            IncrementCustomerPermitResponse permitResp = client.execute(permitReq/*, topsession*/);
    ...
    public Boolean flushTopSession (String id, String token, String session)
    {
        logger.info ("flush the top session");
        try {
    ...
    {
        TaobaoClient client=new DefaultTaobaoClient(new MBaseInfo().url(), new MBaseInfo().appKey(), new MBaseInfo().appSecret());

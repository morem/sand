...
public class MUserData extends Object{
    ...
    public void CreateUser(String id,String nick, String session, Map map) {
        ...
        logger.info ("Create User id:" + id + "nick:" + nick);
    ...
    public String GetUserTopSession(String id)
    {
        ...
        long ts = Long.valueOf(GetUserData(id, "/user/ts"));
        long expires_in = Long.valueOf (GetUserData(id, "/user/expires_in"));
        long cur = datePc.getTime() + new MSyncTime().getOffset ();
        String token = GetUserData(id, "/user/refresh_token");
        String Session =  GetUserData(id, "/user/top_session");
        //if ((cur - ts) >= expires_in*1000)
        //new MTop_API().flushTopSession(id, token, Session);
